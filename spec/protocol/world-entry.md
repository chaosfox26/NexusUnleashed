...
## CLIENT-DERIVED MESSAGE FORMATS (2026-08-21) — deser.py, NF-FREE

Every world-entry message's wire layout, read from the **client's own deserializer** (`fn20` Read
function per opcode, from the factory table `probe-all.json`) with `deser.py` — a recursive decoder of
the client's bit-stream primitives. **Zero captures.** The tool was validated by reproducing the
documented `0x0117` char-list tree byte-for-byte. Bits are LSB-first (client bit-reader convention).
Read RVAs are `WS+<rva>` into 16042 `WildStar64.exe` (imagebase `0x140000000`).

| opcode | Read RVA | wire layout (client-derived) |
|---|---|---|
| **`0x0981`** world-init | `WS+0x8EAA0` | `[u32 count][count × u32 id]` — matches the prior `ServerWorldInit`, now clean-derived |
| **`0x0988`** | `WS+0xA12D0` | `[u32 n1][n1 × {wstr, wstr, u32, u32, u32, 1b}][3b][u32 n2][n2 × {u32, wstr, u32, u32}]` — string-pair data, **NOT** coordinates (the "world payload" label was a size-guess) |
| **`0x098B`** zone blob | `WS+0xA1400` | `[u32 count][count × …]` (element reader not yet expanded) |
| **`0x0262`** entity-create | `WS+0x96FA0` | large nested descriptor: `u32; 6b type; 8b; 5b; array{5b,2b}; u32; 5b; array{5b}; 8b; array{8b}; 7b; array visuals{7b,15b,14b,32b}; 9b; array cmd{3×u32 (position float32×3), 18b, 1b, u32, arrays…}; u32; 14b; 14b; u32 …` — position is 3× float32 inside the `WS+0x94BF0` command sub-block (matches the prior "3× float32" position finding) |
| `0x0966` | `WS+0xA0430` | `[u32 count][array]` |
| `0x0968` | `WS+0x80710` | `[u32 count][array]` |
| `0x00AD` | `WS+0x7E9E0` | `[15b]` (single field / small marker) |
| `0x0987` | `WS+0x7D8E0` | empty marker |
| `0x0036` | `WS+0x7AB50` | trivial/empty |

**Tooling (clean, reusable):** `deser.py` (client Read-fn decoder), `probe-all.json` (opcode→fn10/18/20
factory table, 1121 entries). deser.py path was retargeted to `clients/Wildstar/...` after the
consolidation. To decode any opcode: look up its `fn20` in `probe-all.json`, add `0x140000000`, run
`python deser.py 0x<VA>`.

> **The "player self block" (world-entry.md #9, labeled `0x0117`/833B) is a mislabel** — `0x0117` is the
> char-list. In the world stream the player is placed by a **`0x0262` entity-create for the player's own
> guid** (position packed in the `WS+0x94BF0` command sub-block); "self" is designated separately. To be
> pinned live.
...
> **RETIRED (2026-08-21):** the steps below that say "decode its captured payload / the bytes are in
> `realm-source/captures/`" are **NF reference-poison** and no longer the method. Use the
> **CLIENT-DERIVED MESSAGE FORMATS** section above (deser.py on the client's own Read functions) +
> generate from our DB. The only clean use of the live client is the oracle *loop* (send our
> generated bytes, watch what the client renders/rejects, fix) — never lifting capture bytes.
