...
// --- hysteresis: an entity between enter and leave stays if seen, hides if not ---
{
    ...
    uint near = wi.Add(new Entity { Position = new Vector3(0, 0, 100) });   // inside enter
    uint edge = wi.Add(new Entity { Position = new Vector3(0, 0, 135) });   // between enter and leave
    var p = new PlayerEntity { Position = Vector3.Zero };
    wi.Add(p);
    ...
    wi.Move(p.Guid, new Vector3(0, 0, 20));   // edge now 115 -> enters
    wi.UpdateVision(p);
    Check("edge enters when inside enter radius", p.Visible.Contains(edge));
    wi.Move(p.Guid, Vector3.Zero);            // edge back to 135 (between enter & leave)
    wi.UpdateVision(p);
    Check("edge STAYS visible at 135 once seen (no flicker)", p.Visible.Contains(edge));
    wi.Move(p.Guid, new Vector3(0, 0, -20));  // edge now 155 -> past leave
    wi.UpdateVision(p);
